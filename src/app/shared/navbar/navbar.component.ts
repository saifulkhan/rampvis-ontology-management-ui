...
export class NavbarComponent implements OnInit {
    ...
    getPath(name: string) {
        const pathArray = this.currentPath.split('/').slice(1);
        const index = pathArray.indexOf(name);
        if (index > 0) return pathArray.slice(0, index + 1).join('/');
        return pathArray[0];
    }
    ...
    private setPath(url: string): void {
        ...
        this.pathLinks = this.extractPathLinks(url);
    }

    private extractPathLinks(url: string): Array<string> {
        return url
            .split('/')
            .slice(1)
            .filter((segment) => !segment.match(/\d/g));
    }
}
